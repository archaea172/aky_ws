#!/usr/bin/env python3
"""Visualize a trained PointEnv policy with MuJoCo viewer."""
...
from inrof_mujoco.point_rl import PointEnv


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Open a MuJoCo GUI for a trained PointEnv policy.")
    parser.add_argument("--model", type=Path, default=PROJECT_ROOT / "ppo_point")
    parser.add_argument("--xml", type=Path, default=PROJECT_ROOT / "models" / "point.xml")
...
def main() -> None:
    args = parse_args()
    env = PointEnv(xml_path=args.xml, robot_num=args.robot_num)
    ...
    with viewer.launch_passive(env.model, env.data) as gui:
        ...
        while gui.is_running():
            ...
            if args.print_every and step % args.print_every == 0:
                print(
                    f"step={step:05d} action={np.asarray(action).round(3).tolist()} "
                    f"reward={reward: .3f} mean_dist={info['mean_dist']:.3f}",
                    flush=True,
                )
